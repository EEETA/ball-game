...
window.onload=function(){
    ...
    var launchBall = document.createElement('div');
    launchBall.style={width:'50px',height:'50px',background:'red'}
    document.innerHTML=launchBall;
}
...
function appendball(numberMode){  //生成小球
    ...
    var chooseColor=parseInt(Math.random()*num_number);/*随机选择颜色*/
    box_num=parseInt(Math.random()*box_num_number);/*随机放到各发射器中*/

    // var launchBall = document.createElement('div');

    $('<div class="launchBall" style="background:'+ballFour[chooseColor]+';"></div>').appendTo($('.launcher')[box_num]);
    this_ball=$('.launchBall');
