...
let patternNow;/*当前模式*/
let box_num_number=2;/*默认发球为两个*/
let visited=false;/*进入过娱乐模式后，不再弹出操作提示框*/
...
function appendball(patternNow){  //生成小球
    ...
    let chooseColor=parseInt(Math.random()*box_num_number);/*随机选择颜色*/
    let chooseBox=parseInt(Math.random()*box_num_number);/*随机放到各发射器中*/

    $('<div class="launchBall" style="background:'+ballFour[chooseColor]+';"></div>').appendTo($('.launcher')[chooseBox]);
    var this_ball=$('.launchBall');
    setTimeout(function(){ballmove(this_ball,chooseBox,patternNow)},delayed);
}

/*小球运动*/
function ballmove(this_ball,box_num,patternNow){
    delayed=0;
    box_num < 2 ? (box_num==0 ? range=300 : range=-300) : (box_num==2 ? range=300 : range=-300);
