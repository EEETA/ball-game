...
function initInfo(patternNow){
    ...
    fractionNow=8;
    if(patternNow!=2){fractionNeed=10;}
    else{fractionNeed="∞";}
    speed=1200;
    // initInfo=1;
    balo=1;
...
function appendball(patternNow){  //生成小球
	...
	if(patternNow==1&&fractionNow==60){
        ...
        {[goto]:""+range+"px"},
        {
            ...
            progress:function(){
                ...
                if(launchNum == 4){
                    if(goto=='top'){
                        if(range==350&&Math.floor(launchBallT+launchBallH)>=box_top){
                            movejudge(this,'shang',2,270,90,180,0);/*上往下*/
                        }else if(range==-350&&launchBallT<=box_height){
                            movejudge(this,'xia',2,90,270,0,180);/*下往上*/
                        }
                    }else if(goto=='left'){
                        if(range==350&&Math.floor(launchBallL+launchBallW)>=box_left){
                            movejudge(this,'left',2,0,180,270,90);/*左往右*/
                        }else if(range==-350&&launchBallL<=box_width){
                            movejudge(this,'right',2,180,0,90,270);/*右往左*/
                        }
                    }
                }
            }
