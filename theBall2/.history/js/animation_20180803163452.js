...
/*撞击成功*/
success=function(r,borderSpacing){
    switch_1=1;
    $('<div class="ball_middle" style="background-color:'+$(r).css('background-color')+';display:block"></div>').appendTo(box);
    $('.ball_middle').animate({ borderSpacing:borderSpacing },{
            step:function(now){
...
movejudge=function(r,rou,red,qing,black,num,patternNow){
    numbox=numList;
    if(numList>0){
        numbox=negative;
    }
...
box_animation=function(height,width,speed){
    speed = speed||1000;
    $('.box').animate(
        {
            width:width+'px',
