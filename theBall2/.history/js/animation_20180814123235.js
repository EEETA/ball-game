...
boom=function(div,x,y,color){  //爆炸效果
        $('body').append(div);
        $(div).css({
            left:x+'px',
            top:y+'px',
...
function movejudge(r,rou,red,qing,black,num,patternNow){
    numbox=numList;
    // if(numList>0){
    //     numbox=negative;
    // }
    /*
        ...
        黑色rgb(51, 51, 51);
    */
    shootBallColor=$(r).css('background-color');
    if(shootBallColor=='rgb(51, 51, 51)'&&Math.abs(numbox/180%2)==1){  //黑
        console.log('判定为黑色');
        success(r,num,shootBallColor);
    }else if(shootBallColor=='rgb(244, 86, 70)'&&Math.abs(numbox/180%2)==0){  //红
        console.log('判定为红色');
        success(r,num,shootBallColor);
    }
    else{fail(r)};
...
/*撞击成功*/
function success(r,borderSpacing,shootBallColor){
    // switch_1=1;
    $('<div class="ball_middle" style="background-color:'+shootBallColor+';display:block"></div>').appendTo(ballShow);
    // $('.ball_middle').addClass('ballZhuan123');
    $('.ball_middle').animate(
        {
...
//     $('.Mask_p').fadeIn(800);
//     $('.Mask_span').fadeIn(800);
//     // $('.Mask_p').addClass('play');
// }
